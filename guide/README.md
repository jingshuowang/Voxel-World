...
current problem gpu time should be 0.05 ms and we at 0.5 need enhancement. implement incorretly
plan:
1	Greedy meshing + shader uses w/l	2,500
2	GPU instancing (base quad, instance buffer)	6,500
3+4	Global buffer + multi-draw indirect + SSBO	12,000
5	6 sub-meshes per chunk + face-direction culling	14,000
6	2D greedy meshing (width axis freed by SSBO)	177,000
general rule, call less, render less, less data
Second Part:
...
CPU: this cpu part simple, main thing is collision, dont oevercomplicate, the cpu distance is much less than render distance(2chunks). also render distance and pcpu distance is a shpere, not a cube, a sphere is much more efficient. dont do too mcuh on evaluating if something is in radisu or not though. we use AABB. and jbullet for preciser ones. my favorite is perlin nosie for cpu, its reallly good u can check in the guid folder theres stuf in it, perlin noise is special its fast but kidna sus, it looks, fake. but whatever also theres another one called cellulkar noise. and lastly theres DLA which is costly but looks good, these are for terrain. for mountains 
All of these need to work with multithreading.messes witrh player inputs too, add AI later, and most importantly preparing data for gpu to render.
...
BUG STACK: crouch on ledge is weird, not wroker and when "workiong" i can literally hover in the air. Chunk genration isnt proper, theres holes in the chunks like soem arent rendered. cpu causing way too much lag. Almost no grass for some reason.